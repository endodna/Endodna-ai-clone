...
export function PasswordValidation({ password, confirmPassword }: PasswordValidationProps) {
  ...
  useEffect(() => {
    ...
    if (confirmPassword !== undefined) {
      newValidations['match'] = password === confirmPassword && password.length > 0
    }
    ...
    <div className="mt-2 space-y-1">
      {validationRules.map(rule => (
        <div key={rule.id} className="flex items-center space-x-2">
          ...
          <span className={`text-xs ${validations[rule.id] ? 'text-green-600' : 'text-gray-500'}`}>
            {rule.label}
          </span>
        </div>
      ))}
      ...
      {confirmPassword !== undefined && (
        <div className="flex items-center space-x-2">
          ...
          <span className={`text-xs ${validations['match'] ? 'text-green-600' : 'text-gray-500'}`}>

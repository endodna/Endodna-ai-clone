...
export function AiChatInput({
  ...
  const [persona, setPersona] = useState<(typeof PERSONA_OPTIONS)[number]>(
    patientId ? PERSONA_OPTIONS[0] : PERSONA_OPTIONS[1] // Default to research if no patientId
  );
  ...
  useEffect(() => {
    if (initialPrompt) {
      ...
      autoSubmittedRef.current = false; // Reset when new prompt is set
    }
  }, [initialPrompt]);

  // Auto-submit when autoSubmit is true and prompt is set
  useEffect(() => {
    if (
      autoSubmit &&
      initialPrompt &&
      ...
      // Small delay to ensure prompt is set and component is ready
    ...
    try {
      if (isPatientPersona) {
        ...
        if (existingConversationId && disableChatModal) {
          ...
          // Only open chat modal if not disabled
          if (!disableChatModal) {
            dispatch(
              selectGlobalConversation({
                conversationId,
            ...
            // Notify parent component about conversation creation
            onConversationCreated?.(conversationId, "patient");
          }

          // Set conversation title only for new conversations
  ...
  const handlePersonaSelect = (option: (typeof PERSONA_OPTIONS)[number]) => {
    // Prevent selecting patient persona if no patientId
    if (option.id === "patient" && !patientId) {
      toast.error("Patient ID is required for patient-specific conversations.");
      return;
    }
    setPersona(option);
  ...
  // Filter persona options based on patientId availability
  const availablePersonaOptions = patientId
    ? PERSONA_OPTIONS
    : PERSONA_OPTIONS.filter((option) => option.id !== "patient");
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <div>
              ...
              <Tooltip>
                ...
                <TooltipContent
                  side="top"
                  align="center"
                  className="w-48 rounded-xl bg-primary-foreground px-4 py-4 typo-body-3 text-foreground shadow-[0_12px_30px_rgba(0,0,0,0.25)]"
                  variant="inverted"
                >
            ...
            <div className="flex items-center gap-1 rounded-full border border-muted-foreground bg-primary-foreground px-1 py-1 shadow-sm">
              {availablePersonaOptions.map((option) => {
                const Icon = option.icon;
                const isActive = option.id === persona.id;
                const isDisabled = option.id === "patient" && !patientId;
                return (
                  <Tooltip key={option.id}>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        disabled={isDisabled}
                        className={cn(
                          "flex h-7 w-7 items-center justify-center rounded-full border typo-body-3 transition hover:text-primary",
                          isActive
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="gap-2 rounded-full border border-muted-foreground bg-primary-foreground px-4 py-[6px] typo-body-2  text-foreground hover:bg-primary hover:text-foreground"
                >
              ...
              <DropdownMenuContent align="end" className="w-64">
                {MODEL_OPTIONS.map((option) => (
                  <DropdownMenuItem
                    key={option.id}
                    className="flex gap-3 py-2 items-center group"
                    onClick={() => setSelectedModel(option)}
                  >
                    <option.icon className="h-4 w-4 text-muted-foreground group-data-[highlighted]:text-foreground" />
                    <div className="flex flex-col text-left">
                      <span className="typo-body-2 text-foreground">
                        {option.label}
                      </span>
                      <span className="typo-body-3 text-muted-foreground group-data-[highlighted]:text-foreground">
                        {option.description}
                      </span>
                    </div>
                  </DropdownMenuItem>

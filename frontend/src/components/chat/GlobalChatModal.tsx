...
export function GlobalChatModal() {
  ...
  const currentPatientConversationId =
    conversationType === "patient" && selectedConversationId
      ? selectedConversationId
      : activePatientConversationId;
  const currentGeneralConversationId =
    conversationType === "general" && selectedConversationId
      ? selectedConversationId
      : activeGeneralConversationId;

  const { data: patientMessages, isLoading: isLoadingPatientMessages } =
    useGetPatientConversationMessages(
      selectedPatientId ?? "",
      currentPatientConversationId ?? "",
      {
        enabled:
          ...
          Boolean(currentPatientConversationId) &&
          isGlobalChatModalOpen &&
          conversationType === "patient",
      }
    );
  const { data: generalMessages, isLoading: isLoadingGeneralMessages } =
    useGetGeneralConversationMessages(currentGeneralConversationId ?? "", {
      enabled:
        Boolean(currentGeneralConversationId) &&
        isGlobalChatModalOpen &&
  ...
  const handleChatModalSubmit = async () => {
    ...
    try {
      if (
        ...
        // Invalidate messages query to refresh chat
        queryClient.invalidateQueries({
          queryKey: queryKeys.doctor.chat.general.conversationMessages(
            currentGeneralConversationId
          ),
        });

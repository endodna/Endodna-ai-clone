...
import {
  getDnaResultStatus,
  HoldCancelDiscardResult,
  normalizeDnaStatus,
  StepStatusResult,
} from "@/utils/dnaResult.utils";
import {
  DNA_RESULT_STATUS,
  DNA_TEST_STEPS,
...
const getRowActions = (result: PatientDNAResult): RowAction[] => {
  ...
  // If on hold, can process (resume) or cancel
  ...
  // Get the step status to determine if we're on the last step
  const dnaResultStatus = getDnaResultStatus(result.status);

  // Check if it's the last step (FILE_DELIVERY)
  if (
    dnaResultStatus &&
    ...
    // If on last step (FILE_DELIVERY) or DATA_DELIVERED status, show only "Open" button
    if (
      stepStatus.id === DNA_TEST_STEPS.FILE_DELIVERY ||
      normalizedStatus === DNA_RESULT_STATUS.DATA_DELIVERED
    ) {
      return [
        {
          label: "Open",
  ...
  // Default: not on last step - can cancel or put on hold
  return [
...
const renderSpecialStatus = (
  dnaResultStatus: HoldCancelDiscardResult
): React.ReactNode => {
  if (dnaResultStatus.isHold) {
    return (
      <div>
        <p className="typo-body-2 typo-typo-body-2-regular text-foreground">
          Analysis is paused
        </p>
        <p className="typo-body-3 typo-body-3-regular text-foreground">
          Resume to keep tracking
        </p>
      </div>
    );
  }
...
export const createDnaResultsColumns = (
  patientId: string,
  onOpenClick?: () => void
): ColumnDef<PatientDNAResult>[] => [
  {
    id: "name",
    header: () => (
      <span className="typo-body-2 font-semibold text-foreground">Name</span>
    ),
    cell: ({ row }) => {
  ...
  },
  {
    id: "status",
    header: () => (
      <span className="typo-body-2 font-semibold text-foreground">
        Lab Status
      </span>
    ),
    cell: ({ row }) => {
      ...
      const stepStatus = dnaResultStatus as StepStatusResult;
      return (
        <DnaResultTimeline id={stepStatus.id} status={stepStatus.status} />
      );
    },
  },
  {
    id: "actions",
    header: () => (
      <span className="typo-body-2 font-semibold text-foreground">Actions</span>
    ),
    cell: ({ row }) => {

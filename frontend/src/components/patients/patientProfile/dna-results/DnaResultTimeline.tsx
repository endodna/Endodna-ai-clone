...
import { DNA_TEST_STEPS, DNA_STEP_STATUS } from "@/components/constants/DnaResults";
import type { DnaTestStepId, DnaStepStatus } from "@/components/constants/DnaResults";
...
const Icon = ({ status }: { status: DnaStepStatus }) => {
    ...
    switch (status) {
        case DNA_STEP_STATUS.COMPLETE:
            return <CircleCheckBig className={`h-6 w-6 text-primary ${baseClasses}`} />;
        case DNA_STEP_STATUS.FAILED:
            return <CircleX className={`h-5 w-5 text-destructive ${baseClasses}`} />;
        case DNA_STEP_STATUS.CURRENT:
            return <CircleDot className={`h-5 w-5 text-primary ${baseClasses}`} />;
        default:
            return <Circle className={`h-5 w-5 text-muted-foreground ${baseClasses}`} />;
    }
};

// Build ordered steps with status inferred from provided (active) step id + stepStatus
const buildSteps = (activeId: DnaTestStepId | null, activeStatus: DnaStepStatus) => {
    const steps = DNA_STEPS as readonly { id: DnaTestStepId; title?: string }[];

    // Validate activeStatus is a valid DnaStepStatus
    const validStatus = Object.values(DNA_STEP_STATUS).includes(activeStatus) 
        ? activeStatus 
        : DNA_STEP_STATUS.UPCOMING;
    ...
    return steps.map((s, idx) => {
        ...
        if (activeIndex === -1) {
            ...
            // use the provided activeStatus (could be COMPLETE/CURRENT/FAILED)
            status = validStatus;
        } else {
            status = DNA_STEP_STATUS.UPCOMING;
        }
        ...
        return (
            <ol className="flex flex-col items-start gap-4 py-4">
                {steps.map((step, idx) => {
                    ...
                    } else if (step.status === DNA_STEP_STATUS.FAILED) {
                        connectorClasses = "border-dashed border-destructive";
...
export const DnaResultTimeline: React.FC<StepStatusResult> = ({ id, status }) => {
    if (!id) return null;

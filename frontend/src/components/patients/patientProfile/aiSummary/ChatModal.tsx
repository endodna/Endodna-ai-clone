...
export function ChatModal({
    ...
    const { data: patientMessages, isLoading: isLoadingPatientMessages } = useGetPatientConversationMessages(
        ...
        {
            enabled: Boolean(patientId) && Boolean(currentPatientConversationId) && isChatModalOpen,
        }
    );
    const { data: generalMessages, isLoading: isLoadingGeneralMessages } = useGetGeneralConversationMessages(
        ...
        {
            enabled: Boolean(currentGeneralConversationId) && isChatModalOpen,
        }
    );
    ...
    const currentMessages = conversationType === "patient" || isPatientPersona
        ? patientMessages?.data
        : generalMessages?.data;
    const currentConversationId = conversationType === "patient" || isPatientPersona
        ? currentPatientConversationId
        : currentGeneralConversationId;
    const isLoadingMessages = conversationType === "patient" || isPatientPersona
    ...
    useEffect(() => {
        if (isChatModalOpen && messagesEndRef.current) {
            messagesEndRef.current.scrollIntoView({ behavior: "smooth" });
        }
    }, [currentMessages, isChatModalOpen, isSendingMessage]);
        ...
        try {
            if (isPatientPersona && patientId) {
                ...
                queryClient.invalidateQueries({
                    queryKey: queryKeys.doctor.chat.patient.conversationMessages(patientId, currentConversationId),
                });
                ...
                // Invalidate messages query to refresh chat
                queryClient.invalidateQueries({
                    queryKey: queryKeys.doctor.chat.general.conversationMessages(currentConversationId),
                });
    ...
    return (
        <Dialog open={isChatModalOpen} onOpenChange={(open) => {
            if (!open) {
            ...
            <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0">
                <DialogHeader className="px-6 pt-6 pb-4 border-b border-neutral-200">
                    ...
                    <DialogDescription className="typo-body-2 text-neutral-500-old mt-1">
                        {isPatientPersona ? "Patient-specific conversation" : "General research conversation"}
                    </DialogDescription>
                </DialogHeader>
